    ...
    for (i = 0; i < cells.length; i++) { 
     var cell = cells[i];
      ...
      if (index == 2) {
        ...
        if (cell.children[2] != undefined && cell.children[2].tag == 'text') {
          var meta = cell.children[2].data.trim();
          match = meta.match(/Acc-no: (.+) Size: (.+) (\w\w)/);
          result.accession.formatted = match[1];
          result.size = parseInt(match[2]);
          result.unit = match[3];
          if (result.unit == 'kb') {
            result.kb = result.size;
          } else if (result.unit == 'mb') {
            result.mb = result.size;
          }
        }
      }
